...
class LocationEventtableCalendar extends StatefulWidget {
  ...
  const LocationEventtableCalendar({Key key, this.futureEvents, this.title})
      : super(key: key);
  ...
  Widget _buildEventList() {
    return ListView(
      children: _selectedEvents.map((event) {
        var startTime =
            DateFormat("HH:mm").format(DateTime.parse(event.startDate));
        var endTime = DateFormat("HH:mm")
            .format(DateTime.parse(event.startDate).add(Duration(hours: 1)));
        return Container(
          decoration: BoxDecoration(
          ...
          child: ListTile(
            title: Text("${event.eventName} - $startTime - $endTime"),
